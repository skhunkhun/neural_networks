...
def nnTwo(plt):
    ...
    # Define the neural network class two
    class NeuralNetworkTwo:
        ...
        def backward(self, X, y, A3, A2, A1):
        ...
        def train(self, X, y, learning_rate=0.1, epochs=20, batch_size=32):
            ...
            for epoch in range(epochs):
                ...
                for i in range(0, X_train.shape[0], batch_size):
                    ...
                    # Forward pass
                    y_prediction, A2, A1 = self.forward(X_batch)
                    ...
                    # Backward pass
                    dW1, db1, dW2, db2, dW3, db3 = self.backward(X_batch, y_batch, y_prediction, A2, A1)
                ...
                y_valid_prediction, _, _ = self.forward(X_val)
    ...
    nn = NeuralNetworkTwo(784, 256, 128, 10)

    # Train the neural network using the training data
    nn.train(X_train, y_train)

    # Evaluate the performance of the neural network on the test data
    y_prediction, _, _ = nn.forward(X_test)
    test_acc = np.mean(np.argmax(y_test, axis=1) == np.argmax(y_prediction, axis=1))
    print(f"Test accuracy: {test_acc:.4f}")

    # Add to graph
    plt.plot(x_train_arr, y_train_arr, label="Train 2")
    plt.plot(x_test_arr, y_test_arr, label="Test 2")
